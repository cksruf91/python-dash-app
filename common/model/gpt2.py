...
def get_gtp_model():
    cntnt_meta = get_metadata()
    pad_token_id = cntnt_meta.loc[cntnt_meta.CNTNT_ID == '[PAD]', 'ITEM_ID'].item()
    bos_token_id = cntnt_meta.loc[cntnt_meta.CNTNT_ID == '[BOS]', 'ITEM_ID'].item()
    eos_token_id = cntnt_meta.loc[cntnt_meta.CNTNT_ID == '[EOS]', 'ITEM_ID'].item()
    vocab_size = cntnt_meta.ITEM_ID.max() + 1
